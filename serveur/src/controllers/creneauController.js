...
const showCreneauxByAnimateur = (req, res) => {
    creneau.getCreneauxByAnimateur(req.params.id, (err, results) => {
        if (err) {
            res.send(err);
        } else {
            res.json(results);
...
module.exports = { showCreneaux, showCreneauxById, showCreneauxByDateTime, showCreneauxByDate, showCreneauByDateTime, createCreneau, updateCreneau, deleteCreneau, showCreneauxByAnimateur };