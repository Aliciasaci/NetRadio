...
Emission.updateEmissionById = (data, id, result) => {
    db.query("UPDATE emission SET title = ?, description = ?, genre = ?, idAnimateur = ? WHERE idEmission = ?", [data.title, data.description, data.genre, data.idAnimateur, id], (err, results) => {
        if (err) {
            console.log(err);
            result(err, null);
        } else {
