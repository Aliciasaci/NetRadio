...
Creneau.updateCreneauByDateTime = (data, date, heure, result) => {
    db.query("UPDATE creneau SET idAnimateur = ?, idEmission = ?, idEpisode = ? WHERE date = ? AND heure = ?", [data.idAnimateur, data.idEmission, data.idEpisode, date, heure], (err, results) => {
        if (err) {
            console.log(err);
            result(err, null);
        } else {
...
// Récuperer tous les créneaux d'un certain animateur
Creneau.getCreneauxByAnimateur = (id, result) => {
    db.query("SELECT * FROM creneau as c , emission as em , episode as ep WHERE c.idAnimateur = ? AND em.idEmission = c.idEmission and ep.idEpisode = c.idEpisode order by c.date, c.heure", [id], (err, results) => {
        if (err) {
            console.log(err);
            result(err, null);
        } else {
